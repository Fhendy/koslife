...
@section('breadcrumb', 'Edit Reminder')
@section('page-title', '✏️ Edit Reminder')
@section('page-description', 'Perbarui pengingat Anda')
        ...
        <form action="{{ route('reminders.update', $reminder) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          rows="3"
                          class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                          placeholder="Deskripsikan pengingat">{{ old('description', $reminder->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="type" 
                    ...
                    @foreach(\App\Models\Reminder::getTypes() as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $reminder->type) == $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
