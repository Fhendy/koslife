...
@section('page-title', '✏️ Edit Jadwal')
@section('page-description', 'Perbarui informasi jadwal')
        ...
        <form action="{{ route('schedules.update', $schedule) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" 
                          ...
                          rows="3"
                          class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                          placeholder="Deskripsikan kegiatan">{{ old('description', $schedule->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="category" 
                            ...
                            required
                            class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        <option value="school" {{ old('category', $schedule->category) == 'school' ? 'selected' : '' }}>📚 Sekolah</option>
                        <option value="pkl" {{ old('category', $schedule->category) == 'pkl' ? 'selected' : '' }}>💼 PKL</option>
                        <option value="organization" {{ old('category', $schedule->category) == 'organization' ? 'selected' : '' }}>🤝 Organisasi</option>
                        <option value="meeting" {{ old('category', $schedule->category) == 'meeting' ? 'selected' : '' }}>📅 Meeting</option>
                        <option value="personal" {{ old('category', $schedule->category) == 'personal' ? 'selected' : '' }}>👤 Pribadi</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <select name="reminder_minutes" 
                        id="reminder_minutes"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    <option value="0" {{ old('reminder_minutes', $schedule->reminder_minutes) == 0 ? 'selected' : '' }}>Tidak ada pengingat</option>
                    <option value="5" {{ old('reminder_minutes', $schedule->reminder_minutes) == 5 ? 'selected' : '' }}>5 menit sebelumnya</option>
                    <option value="10" {{ old('reminder_minutes', $schedule->reminder_minutes) == 10 ? 'selected' : '' }}>10 menit sebelumnya</option>
                    <option value="15" {{ old('reminder_minutes', $schedule->reminder_minutes) == 15 ? 'selected' : '' }}>15 menit sebelumnya</option>
                    <option value="30" {{ old('reminder_minutes', $schedule->reminder_minutes) == 30 ? 'selected' : '' }}>30 menit sebelumnya</option>
                    <option value="60" {{ old('reminder_minutes', $schedule->reminder_minutes) == 60 ? 'selected' : '' }}>1 jam sebelumnya</option>
                    <option value="120" {{ old('reminder_minutes', $schedule->reminder_minutes) == 120 ? 'selected' : '' }}>2 jam sebelumnya</option>
                    <option value="1440" {{ old('reminder_minutes', $schedule->reminder_minutes) == 1440 ? 'selected' : '' }}>1 hari sebelumnya</option>
                </select>
            ...
            <div class="flex items-center gap-2">
                <input type="checkbox" 
                       ...
                       id="is_all_day"
                       value="1"
                       {{ old('is_all_day', $schedule->is_all_day) ? 'checked' : '' }}
