...
@section('breadcrumb', 'Tambah Catatan')
@section('page-title', '📝 Tambah Catatan Baru')
@section('page-description', 'Tulis ide, catatan, atau pemikiran Anda')
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach(\App\Models\Note::getColors() as $color => $label)
                        <button type="button" 
                                ...
                                style="background-color: {{ $color }}; border-color: {{ old('color', '#FFFFFF') == $color ? '#4F46E5' : '#E5E7EB' }}">
                        </button>
                    @endforeach
                    <input type="hidden" name="color" id="color_input" value="{{ old('color', '#FFFFFF') }}">
                </div>
                @error('color')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
